...
void setup() {
  ...
  bootstrapManager.bootstrapSetup(NetworkManager::manageDisconnections, NetworkManager::manageHardwareButton,
    ...
    String ldrMaxFromStorage = bootstrapManager.readValueFromFile(ledManager.LDR_CAL_FILENAME,
                                                                  ledManager.MAX_LDR_PARAM);
    String ledOnFromStorage = bootstrapManager.readValueFromFile(ledManager.LDR_FILENAME, ledManager.LED_ON_PARAM);
    String relayPinFromStorage = bootstrapManager.readValueFromFile(ledManager.PIN_FILENAME,
                                                                    ledManager.RELAY_PIN_PARAM);
  ...
  networkManager.getUDPStream();
    ...
    if (ldrEnabled) {
        if (ldrInterval == 0) {
            ...
            if (((currentMillisMainLoop - previousMillisLDR) >= ((ldrInterval * 1000) * 60)) ||
                previousMillisLDR == 0) {
                ldrReading = true;
                previousMillisLDR = currentMillisMainLoop;
            }
