...
function calLDR() {
    if (confirm("Current brightness level of the room will be used as the maximum level from which to start adjusting the brightness of the LED strip. Calibration takes up to 5 seconds and will turn off the LEDs.")) {
        const http = new XMLHttpRequest();
        http.open('GET', '/lights/glowwormluciferin/set?payload={"state":"OFF","effect":"solid","allInstances":1}');
        http.send();
        http.onload = () => callDeviceLdr(1);        
    }  
}
function resetLDR() {
...
sleep(100).then(() => {
    ...
    $('#ldrInterval').append(new Option("5 minutes", "5"));
    $('#ldrInterval').append(new Option("15 minutes", "15"));
    $('#ldrInterval').append(new Option("30 minutes", "30"));
    $('#ldrInterval').append(new Option("45 minutes", "45"));
