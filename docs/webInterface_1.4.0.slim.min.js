...
function asCBAction(cbAs) {
    if (confirm("If auto save is enabled, color and brightness information is stored into memory to retain this settings after reboot. If you change color often, this can stress the small memory on your microcontroller. In that case it is recommended to disable auto save. If you want to save only one color as default, enable auto save, choose the desired color and disable auto save.")) {
        const http = new XMLHttpRequest();    
        if (cbAs.checked) {
